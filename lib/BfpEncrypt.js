...
class BfpEncrypt {
    ...
    async sendData(dataBuffer, senderWif, recipientPublicKey, fileName, fileExt) {
        ...
        // 5 - upload the file
        let fileId = await this.bfp.uploadFile(fundingUtxo, fundingAddress, fundingWif, encryptedEj, fileName, fileExt);
        return fileId
    }

    async fetchEncryptedData(bfpURI, recipientWif) {
        // 1 - download file using URI
        console.log(this.bfp.client)
        ...
        try {
            let encStruct = convertToEncryptStruct(encryptedBuffer)
